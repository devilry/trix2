...
urlpatterns = [
    ...
    re_path(r'^assignments/(?P<assignment_ids>[\d+&*]+)/progress/partial/?$',
            login_required(assignments.AssignmentsProgressPartialView.as_view()),
            name='assignments_progress_partial'),
    re_path(r'^course/(?P<course_id>\d+)/progress/partial/?$',
            login_required(course.CourseProgressPartialView.as_view()),
            name='course_progress_partial'),
